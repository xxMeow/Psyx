...
config.read('../config/service.ini')


#
# Tool : Constants / Config / Validation Functions
#
class Tool():
    ...
    FILE_NAME_LEN_MAX = 32 # < $
    PACKBASE_PATH = config['directory']['packbase_dir']
    LOGBASE_PATH = config['directory']['logbase_dir']
    ...
    def check_file_name(cls, filename):
        if not isinstance(filename, str):
            return False
        if len(filename) >= cls.FILE_NAME_LEN_MAX:
            return False
...
def get_pack_info(p_id): # used to check if pack exists
...
def get_pack_path(sex, age): # used to select pack by sex and age
...
def get_all_packs(): # used to get the list of all packs
...
def get_all_replies(p_id): # used to get all replies of a pack
...
def add_pack(sex, age_min, age_max, pack_name):
    ...
    with db.cursor() as cursor:
        ...
        result = cursor.fetchall()
    db.close()
    if len(result) > 0: # conditions overlapped
        return p_id
    ...
    return p_id # p_id == -1: failed to insert / p_id == 0: conditions overlapped / p_id > 0: success


def delete_pack(p_id):
    sql1 = 'SELECT pack_name FROM pack WHERE p_id=%s' # delete all replies of this pack
